...
def data_process(dataset_name, split_rate=0.9, user_fre_threshold = None, item_fre_threshold = None, socia_data= False, shuffle_split= True, with_time=False, leave_out=None):
    ...
    interact = pd.read_table(save_dir+"/ratings.txt", sep="\t", header= None, names= ['userid', 'itemid', 'score', 'timestamp'])
    if with_time == True:
    ...
    try:
        ...
        user_feature.to_pickle(save_dir + '/encoded_user_feature.pkl')
    except:
        pass
    ...
    try:
        item_feature = pd.read_pickle(save_dir + '/item_feature.pkl')
        item_feature = pd.merge(item_encoder_map,item_feature,how='left',on='item')
        item_feature.to_pickle(save_dir + '/encoded_item_feature.pkl')
    except:
        pass
    ...
    if leave_out == None:
        ...
        for chunk in interact_chunk:
            chunk = chunk[1]
            if len(chunk) > leave_out:
                interact_train.append(chunk[:-leave_out])
                interact_test.append(chunk[-leave_out:])
            else:
                interact_train.append(chunk)
